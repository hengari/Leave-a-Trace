...
纯本地的桌面端个人工作记录工具：每日任务三态记录 + 打卡动效 + 时限渐变/逾期跟进 + 日/周/月热力图 + 月度汇总（生成 PDF/MD）+ 文件库（xlsx/PDF）+ 全局搜索 + JSON 备份恢复。所有数据输入即保存到 localStorage，无云依赖。
...
桌面版在浏览器版基础上补齐系统能力：文件导入自动写入磁盘资料库（默认 `文档/留痕资料库`，可在文件库页切换）、系统软件打开、在文件夹中显示、月报导出 PDF 另存。打包安装包：
...
脚本会启动一个零依赖本地静态服务（`http://127.0.0.1:8787/`）并用默认浏览器打开。相比直接双击 HTML，本地服务提供稳定的存储源，也为后续文件库/月报等系统级能力留好地基。
...
- 存储：localStorage 单键 `trace:v1` 即存 + 磁盘资料库存文件本体
